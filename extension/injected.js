...
(() => {
  ...
  const geminiAdapter = {
    ...
    isSendRequest(url, method) {
      // Gemini uses Google's batchexecute RPC endpoint for turns.
      return method === "POST" && /(StreamGenerate|GenerateContent|batchexecute|assistant\.lamda)/i.test(url);
    },
    extractText(body) {
      // Body is a urlencoded "f.req=<json>" blob, not plain JSON. Pull the
      // longest human-looking string out of it as a best-effort. If nothing is
      // found the caller fails closed (this site is recognized as a send).
      ...
      for (const c of candidates) {
        ...
        if (typeof s === "string" && s.length > best.length && /\s/.test(s)) best = s;
      }
      return best;
    },
  };
  ...
  // Fail-closed: a recognized send whose text can't be extracted is blocked.
  ...
  window.fetch = async function (input, init) {
    ...
    if (adapter.isSendRequest(url, method)) {
      ...
      if (block) {
        throw new DOMException("Bloqueado pelo Claude Guardian", "GuardianBlocked");
      }
  ...
  if (XHR && XHR.prototype) {
    ...
    XHR.prototype.send = function (body) {
      ...
      // Only string bodies can be inspected/redacted here.
      decide(typeof body === "string" ? body : "").then(({ block, redactedBody }) => {
        if (block) {
          // Abort the request so no bytes reach the provider.
          try {
            this.abort();
          } catch {
            /* ignore */
